...
#Find time interval:
intraval_time=[]
data = pickle.load(open('/home/matan/PycharmProjects/physical_curiosity_big_analysis/raw_data_all', 'rb'))
...
for delay in range(0,50):
    ...
    poses = pickle.load(open('/home/matan/PycharmProjects/physical_curiosity_big_analysis/data_of_poses_'+str(delay), 'rb'))
        ...
        for step_id, step in step.items():
            ...
            for section_id in poses[subject_id][step_id].keys():
                ...
                for i, d in enumerate(section['time']):
                    use_angle_matrix=AngleMatrix()

                    use_angle_matrix.matrix=poses[subject_id][step_id]['matrix']

                    use_angle_matrix.skeleton_angles=section['skeleton'][i]

                    use_angle_matrix.calculate_robot_angles()

                    robot_calculation=use_angle_matrix.robot_angles

                    error=np.linalg.norm((robot_calculation-section['to_nao'][i])[(0,1,4,5),])/4
...
ax.set_title('Avg Error between skeleton angles and command to nao angles, in different delays')
plt.show()
